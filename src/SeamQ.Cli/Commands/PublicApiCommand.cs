...
/// <summary>
/// CLI command that generates public API documentation for Angular workspaces.
/// Creates a separate markdown file per Angular project documenting public types,
/// methods, enumerations, and classes.
/// </summary>
public static class PublicApiCommand
{
    public static Command Create(IServiceProvider serviceProvider)
    {
        var pathsArgument = new Argument<string[]>("paths", "One or more workspace root paths to document")
        {
            Arity = ArgumentArity.ZeroOrMore
        };

        var command = new Command("public-api", "Generate public API documentation for Angular projects")
        {
            pathsArgument
        };
        ...
        command.SetHandler(async (paths) =>
        {
            ...
            var generator = serviceProvider.GetRequiredService<IPublicApiGenerator>();
            ...
            try
            {
                ...
                renderer.WriteLine();
                renderer.WriteInfo($"generated {totalFiles} public API doc(s) in {outputDir}");
            }
