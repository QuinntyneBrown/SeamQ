...
/// <summary>
/// CLI command that generates detailed public Interface Control Documents for Angular workspaces.
/// Creates per-project folders with ICD.md and PlantUML diagrams (sequence, class, C4).
/// </summary>
public static class PublicIcdCommand
{
    public static Command Create(IServiceProvider serviceProvider)
    {
        var pathsArgument = new Argument<string[]>("paths", "Workspace paths to generate ICDs for")
        {
            Arity = ArgumentArity.ZeroOrMore
        };

        var command = new Command("public-icd", "Generate detailed public ICD for each project in a workspace")
        {
            pathsArgument
        };
        ...
        command.SetHandler(async (paths) =>
        {
            ...
            var generator = serviceProvider.GetRequiredService<IPublicIcdGenerator>();
            ...
            try
            {
                ...
                if (globalContext.PromptMode)
                {
                    ...
                    foreach (var wsPath in workspacePaths)
                    {
                        ...
                        await promptGen.GenerateAsync(workspace, "public-icd", outputDir);
                    }
                    return;
                }
                ...
                renderer.WriteLine();
                renderer.WriteInfo($"generated {totalFiles} ICD file(s){(pngCount > 0 ? $" + {pngCount} .png" : "")} in {outputDir}");
            }
